...
import { Send, Users, UserCheck, Bell, Calendar, Eye, Trash2, Plus } from 'lucide-react'
...
const Notifications = () => {
  const [notifications, setNotifications] = useState([
    {
      ...
      message: 'Thank you for upgrading to our premium plan. Enjoy all the new features!',
      type: 'all_users',
      ...
      render: (value, notification) => (
        <div className="text-sm">
          ...
          <p className="text-gray-500">
            ...
            ({notification.deliveredCount > 0 ? Math.round((notification.openedCount / notification.deliveredCount) * 100) : 0}%)
          </p>
        </div>
      )
    },
      ...
      render: (value, notification) => (
        <div>
          ...
          <p className="text-xs text-gray-500">
            {notification.sentAt ? `Sent: ${new Date(notification.sentAt).toLocaleDateString()}` : 'Not sent'}
          </p>
        </div>
      )
    },
  ...
  const handleSend = (notification) => {
    if (confirm(`Send notification "${notification.title}" to ${notification.recipientCount} recipients?`)) {
  ...
  const onSubmit = (data) => {
    const newNotification = {
      ...
      recipientCount: data.type === 'all_users' ? 12543 : 
                     data.type === 'role_based' ? 3456 : 150,
      sentCount: 0,
      deliveredCount: 0,
      openedCount: 0,
      status: 'draft',
      ...
      sentAt: null,
      createdBy: 'Admin User',
      targetAudience: data.type === 'all_users' ? 'All Users' :
                     data.type === 'role_based' ? data.roleTarget || 'Premium Users' :
  ...
  const totalRecipients = notifications.reduce((sum, n) => sum + (n.sentCount || 0), 0)
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-yellow-600">{totalDrafts}</p>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              ...
              <div className="flex items-center space-x-4 text-sm text-gray-500">
                ...
                <span>•</span>
                <span>Recipients: {selectedNotification.recipientCount.toLocaleString()}</span>
                <span>•</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Card className="p-4">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      Delivered ({Math.round((selectedNotification.deliveredCount / selectedNotification.sentCount) * 100)}%)
                    </p>
                  </div>
                </Card>
                ...
                <Card className="p-4">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      Opened ({Math.round((selectedNotification.openedCount / selectedNotification.deliveredCount) * 100)}%)
                    </p>
                  </div>
                </Card>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{formatDateTime(selectedNotification.createdAt)}</p>
                  </div>
                </div>
                {selectedNotification.sentAt && (
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{formatDateTime(selectedNotification.sentAt)}</p>
                    </div>
                  </div>
                )}
              </div>
